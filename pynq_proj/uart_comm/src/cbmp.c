...
BMP* bopen(unsigned int sz, unsigned char* byte_stream) {
	BMP* bmp = (BMP*) malloc(sizeof(BMP));
	bmp->file_byte_number = sz;
	bmp->file_byte_contents = byte_stream;
...
BMP* b_deep_copy(BMP* to_copy) {
	...
	copy->file_byte_contents = (unsigned char*) malloc(
			copy->file_byte_number * sizeof(unsigned char));
...
void bwrite(BMP* bmp) {
	...
	char *output = malloc((bmp->file_byte_number * 2 + 1) * sizeof(char));
	...
	for (int i = 0; i < bmp->file_byte_number; i++) {
		sprintf(2 * i + output, "%x", (bmp->file_byte_contents[i] & 0xF0) >> 4);
		sprintf(2 * i + output + 1, "%x", bmp->file_byte_contents[i] & 0x0F);
	}
	output[(bmp->file_byte_number) * 2] = '\0';

	int sz = bmp->file_byte_number;
	sz *= 2;
...
unsigned int _get_int_from_buffer(unsigned int bytes, unsigned int offset,
		unsigned char* buffer) {
	unsigned char* _buffer = (unsigned char*) malloc(
...
unsigned int _get_pixel_array_start(unsigned char* file_byte_contents) {
	return _get_int_from_buffer(PIXEL_ARRAY_START_BYTES,
	PIXEL_ARRAY_START_OFFSET, file_byte_contents);
}
...
void _get_pixel(BMP* bmp, int index, int offset, int channel) {
	unsigned char value = _get_int_from_buffer(sizeof(unsigned char),
			offset + channel, bmp->file_byte_contents);
	switch (channel) {
	case BLUE:
